...
Vue.component('resource-filter', {
    ...
    methods: {
        ...
        changeSource: function changeSource(data) {
            // 内容源改变事件
            var _this = this;
        ...
        changeCategory: function changeCategory(data, flag) {
            // 频道类型改变事件
            var _this = this;
        ...
        handleSelect: function handleSelect(categoryName, item, keyName) {
            // 处理选中输入意见事件
            var categories = this.searchCondition[categoryName];
            var c = _.filter(categories, function (category) {
                // 判断所选项是否已在列表中
        ...
        setWeight: function setWeight(source, thirdId, weight) {
            // 置顶/下沉数据事件
            var _this = this;
