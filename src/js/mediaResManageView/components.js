 ...
 ***********************************************************************************************/
var toolBar =
'<div class="tool-wrapper" :class="fixedClass">' +
  '<span v-if="options.preset === \'preview\'" style="padding: 10px;">' +
    '<el-tag type="success">{{ convertNum2AuditStatus(options.previewBtnOpt.status) }}</el-tag>' +
    '<el-select v-if="versionCount" v-model="options.previewBtnOpt.version" @input="changeVersion" style="width: 280px; margin: 0 10px">' +
      '<el-option v-for="(opt, oIndex) in hlOptions" :label="opt.label" :value="opt.value" :key="opt.value"></el-option>' +
    '</el-select>' +
  '</span>' +
  '<el-button v-for="btn in btnBar" @click="btn.handler" :icon="btn.runIcon" :type="btn.runType" :size="size">{{ btn.runName }}</el-button>' +
'</div>';
...
Vue.component('tool-bar', {
  ...
  mounted: function () {
    ...
    switch (this.options.preset) {
      ...
      default: // 当未传入预置页面时 根据自行传入的按钮列表进行展示
      {
    ...
    if(fixed) { // 当设定了fixed属性 则对按钮栏添加固定样式
  ...
  computed: {
    size: function () { // view页面的按钮为小号 其余的为默认号
      return this.options.preset === 'view' ? 'small' : '';
    }
  },
  methods: {
    getViewBtn: function () { // view页面获取权限按钮的方法
      var _this = this;
                ...
                handler: function () {
                  ...
                  if (selected.length) {
                    _this.$confirm('确定删除选中的记录吗?', '删除提示', { // 提示是否确认删除
                      ...
                      cancelButtonText: '取消',
                      type: 'warning'
                    }).then(function () { // 若确认删除 调用删除接口进行删除
                      var ids = [];
                      var auditField = toolOptions.auditField;
                      var result = {
                        success: true,
    ...
    getPreviewBtn: function () { // 获取preview页面的权限按钮 确定具体需求后再完善
      var _this = this;
      ...
      this.ajax(urls.historyList || 'historyList.html', {
        ...
        _this.ajax($basePath + '/buttonManage/getAuditDetailButton.html', {
          ...
          for (var i = 0; i < btns.length; i++) {
            var btn = btns[i];
            var runComm = btn.runComm;
            if (runComm === 'copy' && !isAllowCopy) { // 当按钮为创建副本 但又不允许创建副本时
              break; // 跳过该按钮
            }
            switch (runComm) {
              case 'claim': { // 认领按钮
    ...
    claim : function (flag) { // 认领(flag为true)&撤销(flag为false)认领按钮事件
      var _this = this;
      var btnOpt = this.options.previewBtnOpt;
      ...
      this.$confirm('您确定要撤销审核吗?', '撤销审核', {
        ...
        _this.ajax(_this.options.urls.delete || 'delHistory.html', {
          ...
          if(data.success) {
            ...
            if(_this.versionCount > 0) { // 有其他版本时 切换到其他版本详情页
...
'<div class="wrapper">' +
  // 查询条件区域 start
  '<div class="search-form-wrapper" v-if="!options.cancelSearch">' +
    '<el-collapse v-model="activeNames" @change="delayCalDataListH">' + // 闭合查询条件区域时 自动更新数据列表高度 以自适应窗口
      '<el-collapse-item name="1">' +
        '<template slot="title">' +
          '查询条件' +
          '<el-button type="warning" size="small" @click.stop="reset" style="float: right; margin: 8px;">重置</el-button>' + // 重置按钮
        '</template>' +
        '<el-form :inline="true">' +
          '<el-form-item :label="para.label" :key="para.name" v-if="para.searched" v-for="(para, key) in searchForm">' + // 遍历传入的参数 对其中searched为true的数据项进行条件查询
            '<el-select v-if="para.options" v-model="para.value" @input="search">' + // 下拉框字段 以是否传入options来区分 改变时会自动执行search方法
              '<el-option v-for="(option, oIndex) in para.options" :label="option.label" :value="option.value" :key="option.value"></el-option>' +
            '</el-select>' +
            '<el-input v-else v-model="para.value" @change="delaySearch"></el-input>' + // 输入框字段 输入完成时会自动执行search方法
          '</el-form-item>' +
        '</el-form>' +
      '</el-collapse-item>' +
    '</el-collapse>' +
  ...
  // 操作按钮区域 end
  // 数据列表区域 start
  '<div class="data-list-wrapper" ref="dataList">' +
    '<el-table ref="table" v-loading="loading" :row-key="options.idField" :data="tableData.rows" :highlight-current-rowborder="options.singleSelect" @selection-change="handleSelectionChange" @row-click="handleRowClick" style="width: 100%" :height="options.dataListH || dataListH">' +
      '<el-table-column type="selection" :reserve-selection="options.reserveSelection" width="55"></el-table-column>' + // 勾选框 可通过reserveSelection来设置是否可分页多选
      '<template v-for="(para, key) in options.tableColumns">' +
        '<el-table-column v-if="para.customFormatter" :fixed="para.fixed" :align="para.align || \'center\'" :prop="key" :label="para.label" :width="para.width">' + // 当传入customFormatter时 根据customFormatter生成列内容
          '<template scope="scope">' +
            '<div v-if="para.operateFn" @click="para.operateFn(scope.row)">' + // 当传入operateFn时 根据operateFn执行点击事件
              '<span v-html="para.customFormatter(scope.row, scope.row[key])"></span>' +
            '</div>' +
            '<span v-else v-html="para.customFormatter(scope.row, scope.row[key])"></span>' +
          '</template>' +
        '</el-table-column>' +
        '<el-table-column v-else :fixed="para.fixed" :align="para.align || \'center\'" :prop="key" :label="para.label" :width="para.width"></el-table-column>' +
      '</template>' +
    '</el-table>' +
  '</div>' +
  // 数据列表区域 end
  // 分页区域 start
  '<div class="page-wrapper" ref="pagination" v-if="!options.cancelPagination">' +
    '<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 30]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="tableData.total"></el-pagination>' +
  '</div>' +
  // 分页区域 end
  // 默认窗口 start
...
Vue.component('data-list', {
  ...
  mounted: function () {
    var _this = this;
    if(!this.options.dataListH) { // 当未设定数据列表高度时 窗口大小变化时 计算数据列表应有高度 达到自适应
  ...
  methods: {
    handleSelectionChange: function (selection) { // 改变记录选中事件
      var _this = this;
    ...
    handleRowClick: function (row, event, column) { // 处理行点击事件
    ...
    delayCalDataListH: _.debounce(function () { // 由于窗口大小或查询区域改变时 大小变化略有延迟 延迟500秒调整数据列表大小
      this.calDataListH();
    }, 500),
    calDataListH: function () { // 计算数据列表高度
      var dataListH = document.body.clientHeight - this.$refs.dataList.offsetTop; // 文档高度 - 数据列表上方高度
      if (!this.options.cancelPagination) { // 若有分页 减去分页高度40
        dataListH -= 40;
      }
      this.dataListH = dataListH;
    },
    handleSizeChange: function (pageSize) { // 切换页面记录大小
      this.pageSize = pageSize;
      this.search(); // 切换后立即查询
    },
    handleCurrentChange: function (currentPage) { // 切换当前页
      this.currentPage = currentPage;
      this.search(); // 切换后立即查询
    },
    delaySearch: _.debounce(function () { // 输入框输入时 采用去抖的方式 保证输入结束后再进行查询
      this.search();
    }, 500),
    ...
    openDefaultWin: function (options) { // 默认窗口打开事件
    ...
    closeDefaultWin: function () { // 默认窗口关闭事件
 ...
 ***********************************************************************************************/
var picList =
'<div v-loading="loading" class="pic-list-wrapper">' +
  // 查询条件区域 start
  '<div class="search-form-wrapper" v-if="!options.cancelSearch">' +
    '<el-collapse v-model="activeNames">' + // 闭合查询条件区域时 自动更新数据列表高度 以自适应窗口
      '<el-collapse-item name="1">' +
        '<template slot="title">' +
          '查询条件' +
          '<el-button type="warning" size="small" @click.stop="reset" style="float: right; margin: 8px;">重置</el-button>' + // 重置按钮
        '</template>' +
        '<el-form :inline="true">' +
          '<el-form-item :label="para.label" :key="para.name" v-for="(para, key) in options.searchParams">' + // 遍历传入的参数 对其中searched为true的数据项进行条件查询
            '<el-select v-if="para.options" v-model="para.value" @input="search">' + // 下拉框字段 以是否传入options来区分 改变时会自动执行search方法
              '<el-option v-for="(option, oIndex) in para.options" :label="option.label" :value="option.value" :key="option.value"></el-option>' +
            '</el-select>' +
            '<el-input v-else v-model="para.value" @change="delaySearch"></el-input>' + // 输入框字段 输入完成时会自动执行search方法
  ...
  '</div>' +
  // 查询条件区域 end
  '<el-row v-for="(row, rIndex) in convertedRows" :key="row[options.idField]">' +
    '<el-col v-for="(column, cIndex) in row" :key="cIndex" :span="24 / columnNum">' +
      '<el-card class="pic-wrapper" style="margin: 10px;" :body-style="{ padding: \'0px\'}">' +
        '<el-radio class="select-handler" v-model="selected[options.idField]" :label="column[options.idField]" :name="options.idField" @click.native="handleSelectionChange(column)" style="margin: 4px;">&nbsp;</el-radio>' +
        '<div style="height: 200px; text-align: center; overflow: hidden;" @click="handleSelectionChange(column)">' + // 图片显示区域
          '<img :src="column[options.picField]" referrerpolicy="no-referrer" style="height: 100%">' + // 为保证爱奇艺图片正常输出 加入referrerpolicy应对防盗链
        '</div>' +
        '<div style="min-height: 40px; padding: 12px;">' + // 文字显示区域
          '<div v-for="(item, iIndex) in options.textItems" :key="iIndex">' +
            '<span v-if="item.customFormatter" v-html="item.customFormatter(column, column[item.field])"></span>' +
            '<span v-else>{{ column[item.field] }}</span>' +
          '</div>' +
        '</div>' +
      '</el-card>' +
    '</el-col>' +
  '</el-row>' +
  '<div class="page-wrapper" ref="pagination">' + // 分页区域
    '<el-pagination v-if="options.tableData" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[12, 24, 36]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="options.tableData.total"></el-pagination>' +
  '</div>' +
'</div>';
...
Vue.component('pic-list', {
  ...
  methods: {
    ...
    convertData: function (rows) { // 将数组转换为二维数组 便于分行的结构遍历
      var _this = this;
      return _.toArray(_.groupBy(rows, function (num, index) { // 将列表按照列数分组并转化成数组
 ...
 ***********************************************************************************************/
var defaultDialog =
'<el-dialog class="default-dialog" :title="dialog.title" :visible="dialog.visible" size="full" :before-close="dialog.handleClose">' +
  '<iframe :src="dialog.src" :height="bodyHeight" width="100%" frameborder="0"></iframe>' +
'</el-dialog>';
 ...
 ***********************************************************************************************/
var customDialog =
'<el-dialog class="custom-dialog" ref="customDialog" :title="dialog.title" :visible="dialog.visible" :size="dialog.size" :before-close="handleClose" :show-close="dialog.showClose" top="5%">' +
  '<div :style="styleObj">' +
    '<slot name="dialogContent"></slot>' +
  '</div>' +
  '<slot name="dialogFooter" ref="dialogFooter"></slot>' +
'</el-dialog>';
 ...
 ***********************************************************************************************/
var ccTableCell =
'<div class="cc-table-cell">' +
  '<p v-if="!type || type === \'text\'">' +
    '{{value | formatter(formatter, row)}}' +
  '</p>' +
  '<a href="#" v-else-if="type === \'link\'" @click="itemClick">' +
    '{{value | formatter(formatter, row)}}' +
  '</a>' +
  '<img :src="value" v-else="type === \'image\'" referrerpolicy="no-referrer">' +
'</div>';
...
var ccTable =
'<div class="cc-table">' +
  '<el-table :row-key="key" border ref="table" :default-sort="defaultSort" :data="tableData" @row-click="rowClickHandle" @selection-change="handleSelectionChange" @select-all="handleAllSelectionChange" @sort-change="sortChange">' +
    '<el-table-column type="selection" :reserve-selection="reserveSelection || false" width="55" v-if="props && props.length != 0"></el-table-column>' +
    '<el-table-column v-for="item in props" :fixed="item.fixed || false" :prop="item.field" :label="item.name" :width="item.width" :sortable="sortable && (item.sort !== void 0? item.sort : \'custom\')" align="center">' +
      '<template scope="scope">' +
        '<cc-table-cell :row="scope.row" :type="item.type" :formatter="item.formatter" :value="scope.row[item.field]" :click-handle="item.handle"></cc-table-cell>' +
      '</template>' +
    '</el-table-column>' +
  '</el-table>' +
  '<el-pagination v-if="paginFlag" @size-change="sizeChangeHandle" @current-change="pageChangeHandle" :current-page="currentPage" :page-sizes="sizes" :page-size="size" layout="total, sizes, prev, pager, next, jumper" :total="total">' +
  '</el-pagination>' +
'</div>';
Vue.component('cc-table', {
  ...
  created () {
    ...
    if (this.multi === void 0) this.multiFlag = false;
    else this.multiFlag = this.multi
  ...
  methods: {
    ...
    // 点击行时响应选择交互
    rowClickHandle (row, e, column) {
      this.$refs.table.toggleRowSelection(row)
    },
    search (field, order) {
      if (!this.url) {
        let data = this.data.concat([])
        this.tableData = data.slice(this.size * (this.currentPage - 1) + 1, this.size * this.currentPage + 1)
        return
      }
      let data = {}
    ...
    sizeChangeHandle (size) {
      this.size = size
      if (this.paginationChange) this.paginationChange()
      else this.search()
    },
    ...
    sortChange ({ column, prop, order }) {
      this.currentPage = 1
      let rule = ''
      if (order === 'descending') rule = order.slice(0, 4)
      else if (order === 'ascending') rule = order.slice(0, 3)
        ...
        '<el-col :span="6" v-for="item in props">' +
          '<el-form-item :label="item.label">' +
            ...
            '<el-select size="small" v-else-if="item.type === \'select\'" v-model="formInline[item.field]" :placeholder="item.placeholder" @change="selectChange" v-ccrule>' +
              ...
              '</el-option>' +
            '</el-select>' +
            '<el-date-picker v-else-if="item.type === \'date\'" size="small" v-model="formInline[item.field]" :type="item.dateType || \'datetime\'" @change="item.changeHandle || void 0" :placeholder="item.placeholder || \'选择日期\'">' +
            '</el-date-picker>' +
            '<el-cascader v-else-if="item.type === \'cascader\'" :options="item.options" @active-item-change="handleItemChange" :props="item.props"></el-cascader>' +
          '</el-form-item>' +
        '</el-col>' +
        '<el-col :span="6">' +
